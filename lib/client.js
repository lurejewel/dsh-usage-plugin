  ...
  factory: (require) => {
    ...
    if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=\"" + cssId + "\"]") === null) {
      ...
      tag.textContent = [
        ...
        ".dshu-trigger-wide{width:100%;height:34px;border-radius:8px;justify-content:flex-start;gap:8px;padding:0 10px;font-size:13px;color:var(--dsw-alias-label-primary);flex:1}",
        ".dshu-trigger-wide .dshu-trigger-label{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}",
    ...
    async function fetchStats() {
      const res = await fetch("/api/dsh-usage/stats", { cache: "no-store" });
    ...
    function UsageTrigger({ t, wide }) {
      ...
      return react.createElement(primitives.Tooltip, {
        ...
        children: react.createElement("button", {
          ...
          className: "dshu-trigger" + (isOpen ? " dshu-active" : "") + (wide ? " dshu-trigger-wide" : ""),
          onClick: toggleOpen,
          children: wide
            ? [
                react.createElement(primitives.IconDataOutline16, { key: "i", size: 16 }),
                react.createElement("span", { key: "s", className: "dshu-trigger-label", children: label }),
              ]
